...
module Mapper
  ...
  # If :type is :symbol, then :date means the string representing a nonterminal symbol.
  # If :type is :literal, then :date means the string representing terminal symbol(s).
  # The depth attribute is reserved for purposes of the mapper.
  Token = Struct.new( :type, :data, :depth )
  
  # The internal representation of the (abstract) language, used for the genotype->phenotype mapping of Grammatical Evolution.
  # See http://www.grammatical-evolution.org/ 
  # 
  # A Grammar instance is created by the parser (e.g. Abnf::Parser) and passed as a main constructor argument of objects subclassing Mapper::Base.
  # The instance of the Grammar class is basically a Hash of { symbol1 => Rule1, symbol2 => Rule2 ... } assignments. each_pair consists of
  # nonterminal symbol[i] and RuleAlt[i] is Mapper::RuleAlt object (the array of all rule alternatives expanding a given symbol).
  # 
  ...
  class Grammar < Hash
    ...
    # The src argument is the instance of any Hash subclass (for deep copying), the start argument is the start symbol definition.
  ...
  # The first and only RuleAlt consists of only one terminal symbol (literal) 'X', the second one is the sequence of:
  ...
  # Rule contains one or more Mapper::RuleAlts, for GE Mapper::Base to select from.
  #
  # For instance:
  #   <expr> ::= X | X+Y | X*Y  
  #   
  ...
  class Rule < Array
    ...
    # Initialize the Rule from the ary argument (i.e from the Enumerable of RuleAlts)
    # The recursive argument will be copied into self.recursivity attribute. 
    def initialize( ary=nil, recursive=nil )
      super ary unless ary.nil?
      @recursivity = recursive
